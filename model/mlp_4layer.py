...
class MLP:
    def __init__(self,input_dim,hidden1,hidden2,hidden3,output_dim):
        ...
        self.relu1 = ReLU()
        self.dropout1 = Dropout(p=0.2)  # 第一层较低的dropout
        self.linear2 = Linear(hidden1,hidden2)
        self.relu2 = ReLU()
        self.dropout2 = Dropout(p=0.3)  # 中间层适中的dropout
        self.linear3 = Linear(hidden2,hidden3)
        self.relu3 = ReLU()
        self.dropout3 = Dropout(p=0.3)  # 中间层适中的dropout
        self.linear4 = Linear(hidden3,output_dim)
    ...
    def forward(self,x,training=True):
        ...
        y_pred = self.softmax.forward(z4)
        return y_pred

    def backward(self,grad_output):
        grad_z4 = self.softmax.backward(grad_output)  
