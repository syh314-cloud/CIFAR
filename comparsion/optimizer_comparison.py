...
from utils.classic_optimizers import SGD, Momentum, Adam, Adagrad
...
class OptimizerExperiment:
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, shuffled_images.shape[0], self.batch_size):
                ...
                if optimizer_name == 'SGD':
                    optimizer.step(self.learning_rate, model.layers)
                else:
    ...
    def save_results(self):
        ...
        filename = f"optimizer_comparison_results_{timestamp}.json"
    ...
    def plot_results(self):
        ...
        fig.suptitle('Classic Optimizer Comparison Results', fontsize=18, fontweight='bold', y=0.98)
        ...
        # 准备表格数据
        table_data = []
        headers = ['Optimizer', '最终Loss', '最佳Val Acc', '测试准确率', 'Steps/Epoch']
        ...
        # 保存图表
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'optimizer_comparison_plots_{timestamp}.png'
