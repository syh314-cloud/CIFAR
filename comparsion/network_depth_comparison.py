...
from model.mlp_2layer import MLP as MLP2
from model.mlp_4layer import MLP as MLP4
from model.mlp_6layer import MLP as MLP6
...
class NetworkDepthExperiment:
    def __init__(self):
        ...
        self.batch_size = 64
        self.hidden_dim = 512  # 固定隐藏层宽度为512
        
        # 要测试的网络配置: (层数, 模型类, 隐藏层配置)
        self.network_configs = {
            '2-Layer': {
                'class': MLP2,
                'params': [train_images.shape[1], self.hidden_dim, train_labels.shape[1]],
                'depth': 2
            },
            '4-Layer': {
                'class': MLP4,
                'params': [train_images.shape[1], self.hidden_dim, self.hidden_dim, self.hidden_dim, train_labels.shape[1]],
                'depth': 4
            },
            '6-Layer': {
                'class': MLP6,
                'params': [train_images.shape[1], self.hidden_dim, self.hidden_dim, self.hidden_dim, 
                          self.hidden_dim, self.hidden_dim, train_labels.shape[1]],
                'depth': 6
            }
        }
    ...
    def train_single_model(self, network_name, network_config):
        """训练单个模型"""
        print(f"\n🔧 开始训练 - Network: {network_name}")
        
        # 重置随机种子确保公平对比
        np.random.seed(self.SEED)
        
        # 创建模型
        model = network_config['class'](*network_config['params'])
        ...
        for epoch in range(self.epochs):
            ...
            if hasattr(model, 'dropout1'):
                if epoch < 20:
                    ...
                    if hasattr(model, 'dropout3'):
                        model.dropout3.p = 0.0
                    if hasattr(model, 'dropout4'):
                        model.dropout4.p = 0.0
                    if hasattr(model, 'dropout5'):
                        model.dropout5.p = 0.0
                else:
                    ...
                    if hasattr(model, 'dropout4'):
                        model.dropout4.p = 0.3
                    if hasattr(model, 'dropout5'):
                        model.dropout5.p = 0.3
            ...
            for i in range(0, shuffled_images.shape[0], self.batch_size):
                ...
                model.zero_grad()
                # 2层MLP的forward不需要training参数
                if network_name == '2-Layer':
    ...
    def run_experiments(self):
        ...
        print(f"🏗️  固定隐藏层宽度: {self.hidden_dim}")
        print(f"🔧 测试网络深度: {list(self.network_configs.keys())}")
        print("-" * 60)
        
        for i, (net_name, net_config) in enumerate(self.network_configs.items()):
            print(f"\n{'='*60}")
            print(f"实验进度: {i+1}/{len(self.network_configs)}")
            
            result = self.train_single_model(net_name, net_config)
            self.results[net_name] = result
    ...
    def save_results(self):
        ...
        filename = f"network_depth_comparison_results_{timestamp}.json"
        
        # 转换numpy数组为列表以便JSON序列化
        results_to_save = {}
        for net_name, result in self.results.items():
    ...
    def plot_results(self):
        ...
        for net_name, result in self.results.items():
            ...
            ax1.plot(sampled_steps, sampled_losses, label=net_name, 
                    color=colors.get(net_name, 'gray'), linewidth=1.5, alpha=0.8)
        ...
        for net_name, result in self.results.items():
            ...
            ax2.plot(val_accuracies_np, label=net_name, 
                    color=colors.get(net_name, 'gray'), linewidth=2, marker='s', markersize=3)
        ...
        # 3. 测试准确率柱状图
        ax3 = axes[1, 0]
        net_names = list(self.results.keys())
        test_accs = [self._to_numpy(self.results[name]['test_accuracy']) for name in net_names]
        
        bar_colors = [colors.get(name, 'gray') for name in net_names]
        bars = ax3.bar(range(len(net_names)), test_accs, 
        ...
        ax3.set_xticklabels(net_names, fontsize=10)
        ...
        # 准备表格数据
        table_data = []
        headers = ['Network', '深度', '参数量', '最终Loss', '最佳Val Acc', '测试准确率']
        
        for net_name in net_names:
            result = self.results[net_name]
            depth = result['depth']
            total_params = result['total_params']
        ...
        table = ax4.table(cellText=table_data, colLabels=headers,
                         ...
                         colWidths=[0.15, 0.1, 0.2, 0.15, 0.2, 0.2])
        ...
        # 保存图表
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'network_depth_comparison_plots_{timestamp}.png'
        ...
        # 打印最佳结果
        best_net = max(self.results.keys(), key=lambda name: self._to_numpy(self.results[name]['test_accuracy']))
        best_acc = self._to_numpy(self.results[best_net]['test_accuracy'])
        print(f"\n🏆 最佳网络深度: {best_net} (测试准确率: {best_acc:.4f})")
...
def main():
    ...
    # 创建实验对象
    experiment = NetworkDepthExperiment()
