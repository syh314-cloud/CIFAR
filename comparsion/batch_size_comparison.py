...
class BatchSizeExperiment:
    ...
    def train_single_model(self, batch_size):
        """训练单个模型"""
        print(f"\n🔧 开始训练 - Batch Size: {batch_size}")
        ...
        val_accuracies = []
        steps_per_epoch = len(range(0, train_images.shape[0], batch_size))
    ...
    def run_experiments(self):
        ...
        print(f"📦 测试Batch Size: {self.batch_sizes}")
        print("-" * 60)
        
        for i, batch_size in enumerate(self.batch_sizes):
            print(f"\n{'='*60}")
            print(f"实验进度: {i+1}/{len(self.batch_sizes)}")
            
            result = self.train_single_model(batch_size)
    ...
    def save_results(self):
        ...
        filename = f"batch_size_comparison_results_{timestamp}.json"
        ...
        for batch_size, result in self.results.items():
            ...
            results_to_save[str(batch_size)] = {
    ...
    def plot_results(self):
        ...
        fig.suptitle('Batch Size Comparison Results', fontsize=18, fontweight='bold', y=0.98)
        ...
        for batch_size, result in self.results.items():
            ...
            ax1.plot(sampled_steps, sampled_losses, label=f'BS={batch_size}', linewidth=1.5, alpha=0.8)
        ...
        for batch_size, result in self.results.items():
            ...
            ax2.plot(val_accuracies_np, label=f'BS={batch_size}', linewidth=2, marker='s', markersize=3)
        ...
        test_accs = [self._to_numpy(self.results[bs]['test_accuracy']) for bs in batch_sizes]
        
        bars = ax3.bar(range(len(batch_sizes)), test_accs, 
        ...
        ax3.set_xticklabels([f'{bs}' for bs in batch_sizes], fontsize=10)
        ...
        # 准备表格数据
        table_data = []
        headers = ['Batch Size', '最终Loss', '最佳Val Acc', '测试准确率', 'Steps/Epoch']
        ...
        # 保存图表
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'batch_size_comparison_plots_{timestamp}.png'
        ...
        # 打印最佳结果
        best_bs = max(self.results.keys(), key=lambda bs: self._to_numpy(self.results[bs]['test_accuracy']))
        best_acc = self._to_numpy(self.results[best_bs]['test_accuracy'])
        print(f"\n🏆 最佳Batch Size: {best_bs} (测试准确率: {best_acc:.4f})")

def main():
    """主函数"""
    print("🎯 Batch Size对比实验")
    print("=" * 60)
